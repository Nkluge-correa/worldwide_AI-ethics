...
MODAL_BODY_TEXT_STYLE = {'font-size': 22,
                         'text-align': 'justify',
                         'text-justify': 'inter-word'}
TITLE_STYLE = {'display': 'inline-block',
               'vertical-align': 'middle'}
TITLE_ICON_STYLE = {'display': 'inline-block',
                    'vertical-align': 'middle',
                    'margin-left': '1rem'}
CARD_STYLE = {'margin-top': '15px', 'border': 0}
CARD_HEADER_STYLE = {'font-size': 30, 'font-weight': 'bold',
                     'text-align': 'center', 'color': '#dc3d87'}
CARD_BODY_STYLE = {'font-size': 18,
                   'color': '#aaaaaa',
                   'overflowY': 'scroll',
                   'height': '400px',
                   'text-align': 'justify',
...
modal_article = html.Div(
    [
        dbc.Button(['Article ', html.I(className="bi bi-file-text-fill")],
                   id='open-body-abstract', outline=OUTLINE_BUTTON, color=OPEN_BUTTON,
                    ...
                    dbc.Row([
                        dbc.Col([
                            dcc.Loading(id='loading_4', type='default', children=[
                                ...
                                ], color='#272b30', outline=False, inverse=True, style=CARD_STYLE)
                            ])
                        ], md=4, style={}),
                        dbc.Col([
                            dcc.Loading(id='loading_5', type='default', children=[
                                ...
                                ], color='#272b30', outline=True, inverse=True, style=CARD_STYLE)
                            ])
                        ], md=4, style={}),
                        dbc.Col([
...
modal_map = html.Div(
    [
        dbc.Button(
            ...
            id='open-body-map', outline=OUTLINE_BUTTON,
...
modal_institution = html.Div(
    [
        dbc.Button(
            ...
            id='open-body-institution', outline=OUTLINE_BUTTON,
            color=OPEN_BUTTON, n_clicks=0, style=STYLE_BUTTON
        ),
        dbc.Modal(
            [
...
modal_gender = html.Div(
    [
        dbc.Button(
            ...
            id='open-body-gender', outline=OUTLINE_BUTTON,
            color=OPEN_BUTTON, n_clicks=0, style=STYLE_BUTTON
        ),
        dbc.Modal(
            [
                ...
                dbc.ModalBody([
                    dcc.Markdown('''**To infer the gender of the authors responsible for the documents in our sample, we performed an analysis based on the names of each author. Given the variety/diversity that names can possess, it was necessary to use automation.**''', style=MODAL_BODY_TEXT_STYLE), html.Br(),
                    dcc.Markdown(
                        '''**To make an accurate inference, it was also necessary to extract (in addition to each author's name) the most likely nationality associated with each name. For this, we used (in addition to the country/origin of each document) [nationalize.io](https://nationalize.io/), an API service that predicts the nationality of a person given their name. After that, we grouped the names of authors who had the same origin/nationality associated with their names.**''', style=MODAL_BODY_TEXT_STYLE),
                    dcc.Markdown('''**Finally, we used the API services of the [genderize.io](https://genderize.io/) platform to infer the gender of each name. Each request was made by providing the name to be inferred and the ISO-2 code of the nationality associated with that name. In the end, $830$ names were extracted from the $200$ documents analyzed. From those names, $558$ unique names were found, each associated with one of $36$ different nationalities.**''', style=MODAL_BODY_TEXT_STYLE, mathjax=True),
                    dcc.Markdown('''**The final count shows that $66\%$ of the sample ($132$ documents) do not identify the authors. The distribution of authors with "_male_" names was favorable for the remainder of our dataset ($549 = 66\%$). $34\%$ ($281$) of these names were inferred as "_female_."**''', style=MODAL_BODY_TEXT_STYLE, mathjax=True)
                ]),
                dbc.ModalFooter(
                    dbc.Button(
                        html.I(className="bi bi-x-circle-fill"),
...
modal_years = html.Div(
    [
        dbc.Button(
            ...
            id='open-body-years', outline=OUTLINE_BUTTON,
            color=OPEN_BUTTON, n_clicks=0, style=STYLE_BUTTON
        ),
        dbc.Modal(
            [
...
modal_nature = html.Div(
    [
        dbc.Button(
            ...
            id='open-body-nature', outline=OUTLINE_BUTTON, color=OPEN_BUTTON,
            n_clicks=0, style=STYLE_BUTTON
        ),
        dbc.Modal(
            [
...
modal_regulation = html.Div(
    [
        dbc.Button(
            ...
            id='open-body-regulation', outline=OUTLINE_BUTTON,
            color=OPEN_BUTTON, n_clicks=0, style=STYLE_BUTTON
        ),
        dbc.Modal(
            [
...
modal_normative = html.Div(
    [
        dbc.Button(
            ...
            id='open-body-normative', outline=OUTLINE_BUTTON,
            color=OPEN_BUTTON, n_clicks=0, style=STYLE_BUTTON
        ),
        dbc.Modal(
            [
...
modal_impact = html.Div(
    [
        dbc.Button(
            ...
            id='open-body-impact', outline=OUTLINE_BUTTON,
            color=OPEN_BUTTON, n_clicks=0, style=STYLE_BUTTON
        ),
        dbc.Modal(
            [
...
modal_divergence = html.Div(
    [
        dbc.Button(
            ...
            id='open-body-divergence', outline=False,
...
app.layout = dbc.Container(
    fluid=True,
    children=[
        html.Div([dcc.Markdown('# `Worldwide AI Ethics`', style=TITLE_STYLE),
                  html.Img(src=dash.get_asset_url(
                      'globe.svg'), height="50px", style=TITLE_ICON_STYLE)],
                 style={'text-align': 'center',
                        'margin-top': '25px',
                        'margin-bottom': '20px'}),
